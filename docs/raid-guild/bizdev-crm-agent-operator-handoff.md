...
Internal agents use ordinary API tokens generated by their human operators from `/en/profile?tab=developer`. Follow the canonical [NextCRM Agent Onboarding](../../public/AGENT_ONBOARDING.md).

- The token represents the user who generated it; no separate agent account or OAuth flow is required for trusted internal agents.
- The operator names the token clearly, gives it an expiration date, stores it in the agent runtime's secret mechanism, and accepts responsibility for its use.
- A lead agent uses the lead-only MCP configuration and the focused [`nextcrm-leads` skill](../../public/skills/nextcrm-leads/SKILL.md) with its `references/` directory intact.
- An agent that genuinely needs broader CRM access uses the general [`SKILL.md`](../../public/SKILL.md) and full MCP endpoint.
- An admin or manager token may work with any non-deleted lead. A standard-user token retains the application's existing assigned, created, and linked-account visibility.
- Use separate tokens for development and production environments.

The lead-only tools are limited to lead configuration, list, search, get, create, update, and dedicated status update. The agent must not use lead deletion, import, conversion, segment mutation, ownership reassignment, campaigns, enrichment, or unrelated CRM tools.

Before creating a lead, the agent searches for likely matches. If a create request times out or has an unknown result, it searches and asks for human review rather than automatically retrying.
