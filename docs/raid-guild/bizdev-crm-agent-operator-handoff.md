...
### BD Agent Lead Pilot Access

For the trusted-team lead pilot, the BD Agent uses an ordinary API token generated by its human operator from `/en/profile?tab=developer`.

- The token represents the user who generated it; no separate agent account or OAuth flow is required for the pilot.
...
- The agent uses the lead-only MCP configuration and client allowlist described in the [BD Agent MCP Lead Pilot](./bd-agent-mcp-onboarding-initiative.md).
...
- A development token is never reused for the production canary.

The approved pilot tools are limited to lead configuration, list, search, get, create, update, and dedicated status update. The agent must not use lead deletion, import, conversion, segment mutation, ownership reassignment, campaigns, enrichment, or unrelated CRM tools.

Before creating a lead, the agent searches for likely matches. If a create request times out or has an unknown result, it searches and asks for human review rather than automatically retrying. Every production create or update is reviewed by a human during the initial canary.
