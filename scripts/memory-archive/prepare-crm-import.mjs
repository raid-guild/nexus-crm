...
const minScore = Number(args.get("min-score") || DEFAULT_MIN_SCORE);
const includeShipped = args.get("include-shipped") === "true";
...
writeCsv(path.join(outputDir, "accounts.csv"), accountRows, [
  "name",
  "status",
  "type",
  "website",
...
const report = [
  ...
  "## Counts",
  "",
  `- Account candidates: ${accountRows.length}`,
  ...
  "## Default Scope",
  "",
  "- Includes RAIDING and PREPARING records only.",
  "- Excludes shipped/lost historical records unless explicitly requested.",
  ...
  ...accountRows.map((row) => [
    `### ${row.name}`,
    "",
    `- CRM status: ${row.crm_status}`,
    `- CRM lifecycle: ${row.crm_lifecycle}`,
    `- Score: ${row.score}`,
    `- Services: ${row.services_required.join(", ") || "n/a"}`,
    `- Internal people: ${row.internal_people.join(", ") || "n/a"}`,
    `- Knowledge doc: ${row.knowledge_doc || "n/a"}`,
    "",
  ].join("\n")),
  "## Review Only",
  "",
